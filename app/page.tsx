...
function TestSupabaseButton() {
  const handleClick = async () => {
    ...
    try {
      ...
      console.log('Button fetch result:', result)
    } catch (err) {
      console.error('Button fetch error:', err)
    }
  }
  return <button onClick={handleClick} style={{ padding: 8, background: '#33589B', color: 'white', borderRadius: 4, margin: 16 }}>Test Supabase Query</button>
}
